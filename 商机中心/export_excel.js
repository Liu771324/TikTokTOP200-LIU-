...
function appendSummarySheet(workbook, payload) {
  ...
  const headers = ['任务名称', '任务编号', '采集时间', '采集状态', '区间覆盖', '类目进度', '达标商品', '增长结果', '失败记录', '耗时（秒）'];
  const summary = [
  ...
  sheet.addRows(rows);
  applyWidths(sheet, [45, 36, 22, 24, 14, 12, 12, 12, 12, 14]);
...
async function generateExcelReport(payload, xlsxPath) {
  ...
  const resultRows = [['#', '类目', '产品名称', '近30天总成交', '昨日成交', '今日成交', '较昨日增长']];
  ...
  const resultSheet = appendDataSheet(workbook, '成交增长', resultRows, [6, 45, 32, 18, 18, 18, 18]);
...
async function generateExcelFromMarkdown(mdPath, xlsxPath) {
  ...
  for (const line of lines) {
    ...
    if (seven) rows.push(seven.slice(1).map((value) => value.trim()));
  ...
  const hasCategory = rows.some((row) => row.length === 7);
  const header = hasCategory
